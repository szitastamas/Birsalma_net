import { API_KEY, SenderID, AppID } from './key.mjs';
...
const isSectionInViewport = el => {
    ...
    return (
        ...
        rect.top <= (window.innerHeight - window.innerHeight * 0.4 || document.documentElement.clientHeight - window.innerHeight * 0.4)
    );
};
...
fruitListPoints.forEach(listpoint =>
...
function submitContact(e) {
    ...
    if (!isChecked()) {
        ...
        alertMsg.innerHTML = `<h2 style="margin-bottom: 2rem">Kedves ${name}!</h2>
            <p>${alertText}</p>
            `;
...
aboutTextCardTitle.forEach(title =>
